...
Ventajas de Heap:
Reserva dinamicamente, el tamaño se decide en ejecución.
Lo que esta en el Heap, vive mas alla que el metodo o funcion donde se ha creado.
...
La máquina virtual de Java lo busca al iniciar la ejecución y comienza a interpretar el programa desde esa función. Aunque existan muchas clases y métodos en el proyecto, la ejecución solo arranca desde este punto concreto. Si la firma no coincide exactamente, Java no lo reconocerá como método de entrada y el programa no podrá iniciarse de forma autónoma.
...
En static no existe this, no se recomienda abusar de el.
...
### Respuesta
En Java, new es el operador que reserva memoria en el heap para un objeto e invoca a su constructor para inicializarlo. El resultado de new es una referencia al objeto recién creado, que puede almacenarse en una variable del tipo de la clase. 

En comparación con C, donde podría usarse malloc para reservar memoria y luego inicializar manualmente la estructura, en Java la reserva y la inicialización quedan unificadas en una sola expresión mediante new y el constructor.
...
### Respuesta
En Java, el paso de argumentos es siempre por valor. Cuando se pasa un objeto como Punto, lo que se copia es la referencia al objeto (no el objeto en sí). Por ello, si dentro del método se modifican los atributos del objeto referenciado (por ejemplo, p.x = 10;), esos cambios sí se observan fuera del método, porque tanto el argumento externo como el parámetro formal apuntan al mismo objeto en el heap. 

En cambio, si dentro del método se reasigna el parámetro a una nueva referencia (p = new Punto(…);), esa reasignación no afecta a la variable original del llamador, ya que la copia local de la referencia es la que cambia.
